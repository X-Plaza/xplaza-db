...
FROM (
    SELECT 
        SUM(p.product_buying_price * (p.quantity + psu.total_sold_unit)::DOUBLE PRECISION) AS total_expense,
        temp_income.total_income, 
        s.shop_id
    FROM (
        SELECT 
            SUM(o.grand_total_price) AS total_income, 
            o.shop_id
        FROM orders o
        GROUP BY o.shop_id
    ) temp_income
...
SELECT 
    ...
    o.date_to_deliver, 
    o.fk_currency_id, 
    COALESCE(o.additional_info, 'N/A') AS additional_info, 
    COALESCE(o.remarks, 'N/A') AS remarks, 
    o.customer_name, 
    o.mobile_no, 
    o.shop_name, 
    st.status_name, 
    ...
    d.person_name, 
    d.contact_no, 
    COALESCE(o.coupon_code, 'N/A') AS coupon_code, 
...
INSERT INTO modules (module_name, module_description) VALUES 
    ('Master Admin Module', 'This module contains all the things a master admin can do'),
    ('Shop Admin Module', 'This module contains all the things a shop admin can do');

INSERT INTO roles (role_name, role_description, fk_module_id) VALUES 
    ('Master Admin', 'Master Admin', 1),
    ('Shop Admin', 'Shop Owner', 2);
