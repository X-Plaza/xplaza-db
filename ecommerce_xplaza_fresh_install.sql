...
CREATE TABLE "bin_orders" (
  ...
  "grand_total_price" float8,
  "delivery_address" varchar,
  "customer_id" int,
  "shop_id" int,
  "delivery_cost_id" int,
  "fk_payment_type_id" int,
  "fk_status_id" int,
  "coupon_id" int,
...
CREATE TABLE "bin_products" (
  ...
  "fk_product_var_type_id" int,
  "product_var_type_value" int,
  "product_buying_price" float8,
  "product_selling_price" float8,
  "fk_currency_id" int,
  "fk_shop_id" int NOT NULL,
  "quantity" int NOT NULL DEFAULT 1,
  "created_by" int,
...
-- 4. FOREIGN KEYS (grouped by table)
...
-- 6. VIEWS (after all tables and triggers)
...
CREATE OR REPLACE VIEW product_by_sold_unit AS 
SELECT row_number() OVER () AS id, p.product_id, p.product_name, COALESCE(SUM(oi.order_item_quantity), 0::bigint) AS total_sold_unit, p.fk_shop_id AS shop_id
FROM products p
...
SELECT temp_revenue.total_expense, temp_revenue.total_income, temp_revenue.total_income - temp_revenue.total_expense AS total_revenue, temp_revenue.shop_id
FROM (SELECT SUM(p.product_buying_price * (p.quantity + psu.total_sold_unit)::double precision) AS total_expense, temp_income.total_income, s.shop_id
      FROM (SELECT SUM(o.grand_total_price) AS total_income, o.fk_shop_id
            FROM orders o
...
FROM (SELECT row_number() OVER () AS id, o.customer_id, o.customer_name, COALESCE(SUM(o.grand_total_price), 0::double precision) AS total_order_amount, o.shop_id AS shop_id
      FROM orders o
...
SELECT row_number() OVER () AS id, oi.product_id, oi.order_item_name AS product_name, COALESCE(SUM(oi.order_item_quantity), 0::bigint) AS monthly_sold_unit, o.shop_id AS shop_id
FROM order_items oi
LEFT JOIN orders o ON oi.fk_order_id = o.order_id
WHERE date_part('month'::text, o.received_time) = date_part('month'::text, CURRENT_DATE)
...
SELECT row_number() OVER () AS id, o.order_id, o.total_price, o.discount_amount, o.net_total, o.grand_total_price, o.delivery_address, o.customer_id, o.shop_id, o.fk_payment_type_id, o.fk_status_id, COALESCE(o.coupon_id, 0) AS coupon_id, o.received_time, o.date_to_deliver, o.fk_currency_id, COALESCE(o.additional_info, 'N/A'::text) AS additional_info, COALESCE(o.remarks, 'N/A'::text) AS remarks, o.customer_name, o.mobile_no, o.shop_name, st.status_name, CONCAT(o.delivery_schedule_start::time, '-', o.delivery_schedule_end::time) AS allotted_time, o.delivery_cost, pt.payment_type_name, d.delivery_id, d.person_name, d.contact_no, COALESCE(o.coupon_code, 'N/A'::character varying::text) AS coupon_code, COALESCE(o.coupon_amount, 0::double precision) AS coupon_amount, CONCAT(oi.order_item_name, ' (', oi.order_item_var_type_value, oi.order_item_var_type_name, ')') AS order_item_name, oi.order_item_category, oi.order_item_quantity, oi.order_item_quantity_type, oi.order_item_unit_price, oi.order_item_total_price, oi.order_item_image, oi.order_item_id, cur.currency_name, cur.currency_sign
FROM orders o
